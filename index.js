...
try {
	if (process.version.slice(1).split('.')[0] < 10) console.error(new Error(`[MusicBot] node v10 or higher is needed, please update`));
	...
	errGen(msg,err) {
		try {
			...
			let errorSend = new Discord.MessageEmbed()
				...
				.addField("User Info",`<@${msg.member.id}> (${msg.member.id})`);
			if (options.errorChannel !== undefined) {
				...
				// run this if there is not an error channel not defined.
				msg.reply("Something Went Wrong, Sorry! Here's the full info for more information;",errorSend)
			}
		} catch (e) {
			console.error(e)
		}
	  ...
	  checkVoice(mem, bot) {
		return new Promise((resolve, reject) => {
		  ...
		  if (bot.voice.channel) {
			if (bot.voice.channelID == mem.voice.channelID) resolve(mem.voice.channel)
	  ...
	  canSkip(member, queue) {
		...
		else if (queue.last === null) return true;
		else if (queue.last.requester === undefined) return true;
	...
	musicbot.skipFunction = (msg, suffix, args) => {
	  ...
	  if (voiceConnection === null || voiceConnection === undefined) return msg.channel.send(musicbot.note('fail', 'No music being played.'));
	  if (voiceConnection && voiceConnection.channel.id != msg.member.voice.channelID) return msg.channel.send(musicbot.note('fail', `You must be in the same voice channel as me.`));
